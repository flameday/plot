...
// 2个低点之间，只能有1个高点
func filter_max(dataClose []float64, flagArr []int) {
		...
		if flagArr[i] == MIN_VALUE_FLAG {
			...
			for j := posMax + 1; j < posMin; j++ {
				if flagArr[j] == MAX_VALUE_FLAG {
					//判断大小
					if dataClose[j] > dataClose[posMax] {
						flagArr[posMax] = 0
						flagArr[j] = MAX_VALUE_FLAG

						posMax = j
					} else if dataClose[j] <= dataClose[posMax] {
						flagArr[j] = 0
					}
				}
			}
...
// 2个高点之间，只能有1个低点
func filter_min(dataClose []float64, flagArr []int) {
	for i := 0; i < len(flagArr); i++ {
